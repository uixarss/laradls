        ...
        <div class="card-header border-0 pt-6">
            ...
            <div class="card-toolbar">
                ...
                <div class="d-flex justify-content-end" data-kt-kearsipan-table-toolbar="base">
                    @if (auth()->user()->can('create-kearsipan'))
                    <!--begin::Add Lokasi-->
            ...
            <table class="table align-middle border rounded table-row-dashed fs-6 g-5" id="kt_table_kearsipan">
                <thead>
                    <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase table-light gs-0">
                        ...
                        @if ((auth()->user()->can('edit-kearsipan') || (auth()->user()->can('delete-kearsipan'))))
                ...
                <tbody class="text-gray-600 fw-bold">
                    ...
                    <tr>
                        ...
                        @if ((auth()->user()->can('edit-kearsipan') || (auth()->user()->can('delete-kearsipan'))))
                        <td class="text-end">
                            @if (auth()->user()->can('update-kearsipan'))
                            <a href="#" class="btn btn-sm btn-icon btn-bg-light btn-active-color-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_edit_confirm_{{ $lokasi->id }}">
                            ...
                            @endif
                            @if (auth()->user()->can('delete-kearsipan'))
                            <!--begin::Menu item-->
                            <a href="#" class="btn btn-sm btn-icon btn-bg-light btn-active-color-danger" data-bs-toggle="modal" data-bs-target="#kt_modal_delete_confirm_{{ $lokasi->id }}">
...
<div class="modal fade" tabindex="-1" id="kt_modal_tambah">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('data-master.kearsipan.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="d-flex justify-content-between py-5 px-6">
                    <div>
                        <h2 class="fw-bold">Tambah Kearsipan</h2>
                    </div>
...
@if (auth()->user()->can('update-kearsipan'))
<!-- begin::ModalEdit -->
<div class="modal fade" tabindex="-1" id="kt_modal_edit_confirm_{{ $lokasi->id }}">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('data-master.kearsipan.update', $lokasi->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="d-flex justify-content-between py-5 px-6">
                    <div>
                        <h2 class="fw-bold">Ubah Kearsipan</h2>
                    </div>
                ...
                <div class="d-grid gap-3 pt-2 pb-5 px-6">
                    <div class="">
                        <label class="required form-label">Kode Kearsipan</label>
                        <input type="text" name="kode" class="form-control" value="{{ $lokasi->kode }}" required>
                    </div>
                    <div class="">
                        <label class="required form-label">Nama Kearsipan</label>
...
@if (auth()->user()->can('delete-kearsipan'))
<!-- begin::ModalDelete -->
<div class="modal fade" tabindex="-1" id="kt_modal_delete_confirm_{{ $lokasi->id }}">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('data-master.kearsipan.destroy', $lokasi->id) }}" method="POST">
                @csrf
                @method('delete')
                <div class="d-flex justify-content-between py-5 px-6">
                    <div>
                        <h2 class="fw-bold">Hapus {{ $lokasi->name }}</h2>
                    </div>
